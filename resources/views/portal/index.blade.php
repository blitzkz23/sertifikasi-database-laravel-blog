...
@section('slider')
    <div class="main-slider">
        ...
        <div class="px-2 bg-light" id="running-text">
            <marquee class="py-3" direction="left" onmouseover="this.stop()" onmouseout="this.start()"
                scrollamount="10" behavior="scroll">
                @foreach ($data['runningtexts'] as $rtext)
                    <a href="{{ $rtext->link }}" class="text-dark">{{ $rtext->title }} ● </a>
                @endforeach
            </marquee>
        </div>
        <div id="slider">
...
@section('content')
    @foreach ($data['headline'] as $headline)
        ...
        <div class="single-post">
            ...
            <div class="icons">
                <div class="left-area">
                    <a class="btn caegory-btn"
                        href="{{ url('category/' . $headline->category->id) }}"><b>{{ $headline->category->name }}</b></a>
                </div>
            ...
            <h3 class="title"><a href="{{ url('post-detail/' . $headline->id) }}"><b
                        class="light-color">{{ $headline->title }}</b></a></h3>
            ...
            <div class="col-lg-6 col-md-12">
                <div class="single-post">
                    ...
                    <div class="icons">
                        <div class="left-area">
                            <a class="btn caegory-btn"
                                href="{{ url('category/' . $posts->category->id) }}"><b>{{ $posts->category->name }}</b></a>
                        </div>
                        {{-- <ul class="right-area social-icons">
                    <li><a href="#"><i class="ion-android-textsms"></i>06</a></li>
                </ul> --}}
                    ...
                    <h3 class="title"><a href="{{ url('post-detail/' . $posts->id) }}"><b
                                class="light-color">{{ $posts->title }}</b></a></h3>
