...
<body>
    ...
    <header>
        ...
        <div class="middle-menu center-text"><a href="#" class="logo"><img
                    src="{{ url('assets/images/logo.png') }}" alt="Logo Image"></a></div>
        ...
        <div class="bottom-area">
            ...
            <ul class="main-menu visible-on-click" id="main-menu">
                @php
                    $data['mainmenu'] = DB::table('mainmenus')
                        ->where('status', 1)
                        ->where('parent', 0)
                        ->orderBy('order', 'asc')
                        ->get();
                ...
                @foreach ($data['mainmenu'] as $menu)
                    @php
                        $data['mainmenu2'] = DB::table('mainmenus')
                            ->where('status', 1)
                    ...
                    <div class="sidebar-area">
                        ...
                        <div class="sidebar-section src-area">
                            ...
                            <form action="{{ url('search') }}" method="GET">
                                ...
                                <button class="src-btn" type="submit"><i
                        ...
                        <div class="sidebar-section about-author center-text">
                            <div class="author-image"><img src="{{ url($data['user']->image) }}"
                                    alt="{{ $data['user']->name }}"></div>
                            <hr>
                        ...
                        <div class="sidebar-section latest-post-area">
                            ...
                            @foreach ($data['latestposts'] as $posts)
                                <div class="latest-post" href="{{ url('post-detail/' . $posts->id) }}">
                                    <div class="l-post-image"><img src="{{ url($posts->thumbnail) }}"
                                            alt="Category Image"></div>
                                    <div class="post-info">
                                        <a class="btn category-btn"
                                            href="{{ url('category/' . $posts->category->id) }}">{{ $posts->category->name }}</a>
                                        <h5><a href="{{ url('post-detail/' . $posts->id) }}"><b
                                                    class="light-color">{{ $posts->title }}</b></a></h5>
                                        <h6 class="date">
                                            <em>{{ date('D, M Y', strtotime($posts->created_at)) }}</em>
                                        </h6>
                                    </div>
                                </div>
                            @endforeach
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <div class="footer-section">
                        <p class="copyright">Naufal Aldy Pradana | &copy; 2021. All rights reserved</p>
                    </div><!-- footer-section -->
